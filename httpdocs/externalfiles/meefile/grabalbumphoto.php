...
if (($uid==$id) || (mysql_result(mysql_query("SELECT COUNT(*) FROM album_photos_vis WHERE ap_id='$apid' AND type='pub' AND sub_type='y' LIMIT 1"), 0)>0) || (mysql_result(mysql_query("SELECT COUNT(*) FROM ap_tags WHERE ap_id='$apid' AND u_id='$id' LIMIT 1"), 0)>0) || (((mysql_result(mysql_query("SELECT COUNT(*) FROM peep_streams ps INNER JOIN album_photos_vis apv ON (apv.ap_id='$apid'AND apv.type='strm' AND ps.stream=apv.sub_type) WHERE ps.u_id='$uid' AND ps.p_id='$id' LIMIT 1"), 0)>0) || (mysql_result(mysql_query("SELECT COUNT(*) FROM mpc_mems mpcm INNER JOIN album_photos_vis apv ON (apv.ap_id='$apid'AND apv.type='chan' AND apv.ref_id=mpcm.mpc_id) WHERE mpcm.p_id='$id' LIMIT 1"), 0)>0)) && (mysql_result(mysql_query("SELECT COUNT(*) FROM album_photos_vis WHERE ap_id='$apid' AND type='user' AND ref_id='$id' LIMIT 1"), 0)<1))) {
	...
	if ($uid==$id) {
		$pnphotofinds = mysql_query ("SELECT ap_id FROM album_photos WHERE pa_id='$aid' ORDER BY p_num ASC");
	} else {
		$pnphotofinds = mysql_query ("(SELECT DISTINCT ap.ap_id, ap.p_num FROM album_photos ap INNER JOIN album_photos_vis apv ON ap.ap_id=apv.ap_id LEFT JOIN my_peeple mp ON mp.u_id='$uid' AND mp.p_id='$id' LEFT JOIN peep_streams ps ON ps.u_id='$uid' AND ps.p_id=mp.p_id LEFT JOIN mpc_mems mpc ON mp.p_id=mpc.p_id LEFT JOIN album_photos_vis apv2 ON ap.ap_id=apv2.ap_id AND apv2.type='user' AND apv2.ref_id='$id' WHERE (ap.pa_id='$aid') AND ((apv.type='pub' AND apv.sub_type='y') OR (((apv.type='strm' AND apv.sub_type=ps.stream) OR (apv.type='chan' AND apv.ref_id=mpc.mpc_id)) AND (apv2.apvis_id IS NULL)))) UNION DISTINCT (SELECT DISTINCT ap.ap_id, ap.p_num FROM album_photos ap INNER JOIN ap_tags apt ON ap.ap_id=apt.ap_id AND apt.u_id='$id' WHERE ap.pa_id='$aid') ORDER BY p_num ASC");
	}
	$prevap = 0;
	$metthisphoto = false;
	$nextap = 0;
	...
	if ($prevap==0) {
		if ($uid==$id) {
			$prevap = mysql_result(mysql_query("SELECT ap_id FROM album_photos WHERE pa_id='$aid' ORDER BY p_num DESC LIMIT 1"), 0);
		} else {
			$prevap = mysql_result(mysql_query("(SELECT DISTINCT ap.ap_id, ap.p_num FROM album_photos ap INNER JOIN album_photos_vis apv ON ap.ap_id=apv.ap_id LEFT JOIN my_peeple mp ON mp.u_id='$uid' AND mp.p_id='$id' LEFT JOIN peep_streams ps ON ps.u_id='$uid' AND ps.p_id=mp.p_id LEFT JOIN mpc_mems mpc ON mp.p_id=mpc.p_id LEFT JOIN album_photos_vis apv2 ON ap.ap_id=apv2.ap_id AND apv2.type='user' AND apv2.ref_id='$id' WHERE (ap.pa_id='$aid') AND ((apv.type='pub' AND apv.sub_type='y') OR (((apv.type='strm' AND apv.sub_type=ps.stream) OR (apv.type='chan' AND apv.ref_id=mpc.mpc_id)) AND (apv2.apvis_id IS NULL)))) UNION DISTINCT (SELECT DISTINCT ap.ap_id, ap.p_num FROM album_photos ap INNER JOIN ap_tags apt ON ap.ap_id=apt.ap_id AND apt.u_id='$id' WHERE ap.pa_id='$aid') ORDER BY p_num DESC LIMIT 1"), 0);
		}
	}
	if ($nextap==0) {
		if ($uid==$id) {
			$nextap = mysql_result(mysql_query("SELECT ap_id FROM album_photos WHERE pa_id='$aid' ORDER BY p_num ASC LIMIT 1"), 0);
		} else {
			$nextap = mysql_result(mysql_query("(SELECT DISTINCT ap.ap_id, ap.p_num FROM album_photos ap INNER JOIN album_photos_vis apv ON ap.ap_id=apv.ap_id LEFT JOIN my_peeple mp ON mp.u_id='$uid' AND mp.p_id='$id' LEFT JOIN peep_streams ps ON ps.u_id='$uid' AND ps.p_id=mp.p_id LEFT JOIN mpc_mems mpc ON mp.p_id=mpc.p_id LEFT JOIN album_photos_vis apv2 ON ap.ap_id=apv2.ap_id AND apv2.type='user' AND apv2.ref_id='$id' WHERE (ap.pa_id='$aid') AND ((apv.type='pub' AND apv.sub_type='y') OR (((apv.type='strm' AND apv.sub_type=ps.stream) OR (apv.type='chan' AND apv.ref_id=mpc.mpc_id)) AND (apv2.apvis_id IS NULL)))) UNION DISTINCT (SELECT DISTINCT ap.ap_id, ap.p_num FROM album_photos ap INNER JOIN ap_tags apt ON ap.ap_id=apt.ap_id AND apt.u_id='$id' WHERE ap.pa_id='$aid') ORDER BY p_num ASC LIMIT 1"), 0);
		}
	}
	...
	echo '<div align="right" style="width: 666px; margin-bottom: 2px;">
		...
		<div align="left" id="painfoarea"><table cellpadding="0" cellspacing="0" width="666px"><tr><td align="left" valign="center" class="subtext" >
			...
			<table cellpadding="0" cellspacing="0"><tr><td align="center" valign="center"><div id="prevbtn" apid="'.$prevap.'" class="photoview_pnbtns" onclick="backcontrol.setState(\'apid='.$prevap.'\');">PREVIOUS</div></td><td align="center" valign="center">|</td><td align="center" valign="center"><div id="nextbtn" apid="'.$nextap.'" class="photoview_pnbtns" onclick="backcontrol.setState(\'apid='.$nextap.'\');">NEXT</div></td></tr></table>
		</td></tr></table></div>
	</div>';
	...
	echo '<div align="left">
		<table cellpadding="0" cellspacing="0"><tr><td align="center" valign="top" width="664px" style="padding-right: 8px;">';
			...
			if (($uid==$id)||(mysql_result(mysql_query("SELECT COUNT(*) FROM ap_tags WHERE ap_id='$apid' AND u_id='$id' LIMIT 1"), 0)>0)) {
				...
				<div align="left" style="padding-top: 4px; padding-left: 54px;">
					...
					if ($uid==$id) {
						echo '<div align="left" style="width: 120px; padding-top: 12px;"><input type="button" value="visibility" onclick="PopBox.fromElement(this , {url: \''.$baseincpat.'externalfiles/photos/editapvis.php?id='.$apid.'\', size: {x: 660, y: 340}, handler:\'iframe\'});"/></div>
						<div align="left" style="width: 120px; padding-top: 12px;"><input type="button" align="center" valign="center" value="delete" onclick="parent.PopBox.fromElement(this , {url: \''.$baseincpat.'externalfiles/photos/deleteap.php?id='.$apid.'\', size: {x: 660, y: 340}, handler:\'iframe\'});"/></div>';
					}
				echo '</div>';
			}
		echo '</td></tr></table>
