...
$choose = $_GET['choose'];
	...
	if ($choose=='photos') {
		...
		if ($_GET['grab']=='aps') {
			...
			if ($_GET['method']=='cncl') {
				...
				echo '<table cellpadding="0" cellspacing="0"><tr><td align="left" calss="paragraph60">Are you sure?</td></tr><tr><td align="center" style="padding-top: 6px; padding-top: 4px;">
					<form action="'.$baseincpat.'externalfiles/meefile/editmeepic.php?action=setui&uiid='.$uiid; echo'" method="post">
						<table cellpadding="0" cellspacing="0"><tr><td align="left"><input type="submit" name="yes" value="yes"/><td align="left" style="padding-left: 4px;"><input type="button" class="endblur" name="no" value="no" onclick="gotopage(\'ap'.$uiid.'\', \''.$baseincpat.'externalfiles/meefile/editmeepic.php?choose=photos&action=setui&method=cncl&uiid='.$uiid; echo'\', \'65\');"/></td></tr></table>
					</form>
				</td></tr></table>';
			}
				...
				<table cellpadding="0" cellspacing="0">';
					...
					if (@mysql_num_rows($uis)>0) {
						...
						echo '<tr><td align="left" style="border-bottom: 1px solid #C5C5C5; padding-top: 4px;"></td></tr><tr><td align="left" style="border-bottom: 1px solid #ECECEC; padding-top: 12px; padding-bottom: 6px; cursor: pointer;" onclick="gotopage(\'pacatalog\', \''.$baseincpat.'externalfiles/meefile/editmeepic.php?choose=photos&grab=uis\', \'182\');">
								...
								<table cellpadding="0" cellspacing="0"><tr><td align="left" class="crumb14">MeePics <span class="subtext">('.$uicount[0].' Photo'; if($uicount[0]>1){echo's';} echo')</span></td></tr><tr><td align="left"><span class="subtext">These are all of your MeePics.</span></td></tr></table>
							</td></tr></table>		
						</td></tr>';
					$i++;
					}
			...
			reporterror('meefile/editmeepic.php', 'loading swfuploader', 'unable to load, displayed fallback option');
			echo '</span></div><div align="left" style="width: 600px; margin-top: 12px; margin-bottom: 12px;"><span style="font-size: 14px;">Please <a href="http://www.mozilla.com/plugincheck/" target = "_new">check to make sure you have the most recent version of flash installed</a> and try again.</span><br /><span style="font-size: 14px;">If you\'d like, <span class="palert">this is our fallback option &mdash; it takes noticeably longer, so please be patient.</span></span></div>
			<div style="overflow-y: hidden; overflow-x: hidden; height: 150px; width: 444px;">
					<form enctype="multipart/form-data" action="'.$baseincpat.'externalfiles/meefile/editmeepic.php?action=upload" method="post">
						<table cellpadding="0" cellspacing="0" width="300px">
							...
							<tr><td align="center" class="subtext" style="padding-top: 6px;">you may upload a 4MB or smaller photo</td></tr>
...
<script type="text/javascript">
		...
		window.onload = function () {
			swfu = new SWFUpload({
				// Backend Settings
				upload_url: "editmeepic-uploadhandler.php",
				...
				swfupload_load_failed_handler : function(){gotopage(\'mainarea\', \''.$baseincpat.'externalfiles/meefile/editmeepic.php?choose=upload\');},
				file_queue_error_handler : fileQueueError,
				file_dialog_complete_handler : fileDialogComplete,
				upload_progress_handler : uploadProgress,
				upload_error_handler : uploadError,
				upload_success_handler : function(){return true;},
				upload_complete_handler : function(){$(\'uploadermaincont\').set(\'html\', \'<table cellpadding="0" cellspacing="0"><tr><td align="left" class="paragraph60">Your MeePic has been saved.</td></tr></table>\');setTimeout("window.location.replace(\''.$baseincpat.'externalfiles/meefile/editmeepic.php?action=edtthumb\');", 800);},
...
<div align="center" id="mainarea">';
	...
	if ($_GET['action']=='edtthumb') {
		...
		if(isset($_FILES['upload'])) {
				...
				echo '<div align="left" style="margin-bottom: 12px; margin-left: 16px;">
				<table cellpadding="0" cellspacing="0"><tr><td align="left" class="p24" valign="center">Upload |</td><td align="left" valign="top"
				<div align="left" id="btm_btn" class="blockbtn" style="width: 100px;" onclick="window.location.replace(\''.$baseincpat.'externalfiles/meefile/editmeepic.php\');">back to main</div>
				</td></tr></table>
				<div align="left">Please select a photo to upload and set as your new MeePic.</div>
			</div>
			<div align="center" id="uploadermaincont" style="overflow-y: hidden; overflow-x: hidden; margin-top: 24px; height: 124px; width: 600px;">
				<form>
		...
		//set ui photo
			$uiid = strip_tags(escape_data($_GET['uiid']));
					...
					if (mysql_num_rows($photoq)>0) {
						...
						$pfn = strtolower(substr($photo['img_url'], (strrpos($photo['img_url'], 'meepics/')+8)));
			...
			$delete = @mysql_query ("DELETE FROM user_imgs WHERE u_id='$id' AND img_url='$oldurl'");
	...
	} elseif ($_GET['action']=='setapmeepic') {
			//set ap photo
			$apid = strip_tags(escape_data($_GET['apid']));
			$photo = @mysql_fetch_array (@mysql_query ("SELECT pa_id, url FROM album_photos WHERE ap_id='$apid' LIMIT 1"), MYSQL_ASSOC);
			$paid = $photo['pa_id'];
			...
			//test vis
			if ((mysql_result(mysql_query("SELECT COUNT(*) FROM album_photos WHERE pa_id='$paid' AND u_id='$id' LIMIT 1"), 0)>0) || (mysql_result(mysql_query("SELECT COUNT(*) FROM album_photos_vis WHERE ap_id='$apid' AND type='pub' AND sub_type='y' LIMIT 1"), 0)>0) || (mysql_result(mysql_query("SELECT COUNT(*) FROM ap_tags WHERE ap_id='$apid' AND u_id='$id' LIMIT 1"), 0)>0) || (((mysql_result(mysql_query("SELECT COUNT(*) FROM peep_streams ps INNER JOIN album_photos_vis apv ON (apv.ap_id='$apid'AND apv.type='strm' AND ps.stream=apv.sub_type) WHERE ps.u_id='$uid' AND ps.p_id='$id' LIMIT 1"), 0)>0) || (mysql_result(mysql_query("SELECT COUNT(*) FROM mpc_mems mpcm INNER JOIN album_photos_vis apv ON (apv.ap_id='$apid'AND apv.type='chan' AND apv.ref_id=mpcm.mpc_id) WHERE mpcm.p_id='$id' LIMIT 1"), 0)>0)) && (mysql_result(mysql_query("SELECT COUNT(*) FROM album_photos_vis WHERE ap_id='$apid' AND type='user' AND ref_id='$id' LIMIT 1"), 0)<1))) {
			echo '<table cellpadding="0" cellspacing="0"><tr><td align="left">Are you sure you want to set this photo as your MeePic?</td></tr><tr><td align="center" style="padding-top: 6px; padding-top: 4px;">
					<form action="'.$baseincpat.'externalfiles/meefile/editmeepic.php?action=setap&apid='.$apid.'" method="post">
						<table cellpadding="0" cellspacing="0"><tr><td align="left"><input type="submit" name="yes" value="yes"/></td><td align="left" style="padding-left: 4px;"><input type="button" class="endblur" name="no" value="no" onclick="parent.PopBox.close();"/></td></tr></table>
				</form>
			</td></tr></table>';
			} else {
				echo 'You are unable to set this photo as your MeePic.';	
			}
	} elseif ($_GET['action']=='setuimeepic') {
			$uiid = strip_tags(escape_data($_GET['uiid']));
						
			if (mysql_result(mysql_query("SELECT COUNT(*) FROM user_imgs WHERE ui_id='$uiid' AND u_id='$id' LIMIT 1"), 0)>0) {
			echo '<table cellpadding="0" cellspacing="0"><tr><td align="left">Are you sure you want to set this photo as your MeePic?</td></tr><tr><td align="center" style="padding-top: 6px; padding-top: 4px;">
					<form action="'.$baseincpat.'externalfiles/meefile/editmeepic.php?action=setui&uiid='.$uiid; echo'" method="post">
						<table cellpadding="0" cellspacing="0"><tr><td align="left"><input type="submit" name="yes" value="yes"/><td align="left" style="padding-left: 4px;"><input type="button" class="endblur" name="no" value="no" onclick="parent.PopBox.close();"/></td></tr></table>
					</form>
			</td></tr></table>';
			} else {
				echo 'You are unable to set this photo as your MeePic.';	
			...
			<table cellpadding="0" cellspacing="0"><tr><td align="left" valign="top" style="padding-right: 4px;">
				...
				<div class="blockbtn" style="width: 132px; height: 96px;" onclick="window.location.replace(\''.$baseincpat.'externalfiles/meefile/editmeepic.php?action=upload\');">
