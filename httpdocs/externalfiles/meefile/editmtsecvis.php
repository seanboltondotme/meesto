...
$mtinfo = @mysql_fetch_array (@mysql_query ("SELECT mt.u_id, mt.mt_id, mts.title FROM meefile_tab_sec mts INNER JOIN meefile_tab mt ON mt.mt_id=mts.mt_id AND mt.u_id='$id' WHERE mts.mts_id='$mtsid' LIMIT 1"), MYSQL_ASSOC);

echo '<div align="left" class="p24" style="width: 460px; border-bottom: 1px solid #C5C5C5;">Edit "'.ucwords($mtinfo['title']).'" Visibility</div>
...
if (isset($_POST['save'])) {
	...
	if (empty($errors)) {
		...
		reporterror('meefile/editmtsecvis.php', 'editing '.$mtsid.' tab settings', $errors);
		echo '</td></tr></table>
		<script type="text/javascript">
	...
	if (mysql_result (mysql_query("SELECT COUNT(*) FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='pub' LIMIT 1"), 0)>0) {
		$ispub = true;
	} else {
		$ispub = false;
	}
	$plstrm = mysql_query("SELECT sub_type FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='strm'");
	$plstrms = array();
	while ($plstrminfo = mysql_fetch_array ($plstrm, MYSQL_ASSOC)) {
		array_push($plstrms, $plstrminfo['sub_type']);
	}
	$plchan = mysql_query("SELECT ref_id FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='chan'");
	$plchans = array();
	while ($plchaninfo = mysql_fetch_array ($plchan, MYSQL_ASSOC)) {
		array_push($plchans, $plchaninfo['ref_id']);
	}
	
	echo '<form action="'.$baseincpat.'externalfiles/meefile/editmtsecvis.php?id='.$mtsid.'" method="post">
