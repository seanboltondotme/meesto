...
if (!isset($_FILES["Filedata"]) || !is_uploaded_file($_FILES["Filedata"]["tmp_name"]) || $_FILES["Filedata"]["error"] != 0) { //or no $id !important and fix for all uploadhandlers
	echo "ERROR:invalid upload";
	session_write_close();
	exit();
}
//end swf upload params and settings
					$mtsid = escape_data($_POST['mtsid']);
				...
				if (in_array($mimetest['mime'], $allowedmime)) {
						...
						$pfn = strtolower(substr($tmpfn, 0, strrpos($tmpfn, '.')));
									...
									if (move_uploaded_file($filename, "../../users/$id/attachments/$fn")) {
										...
										echo 'mtsedit'.$mtsid.'&users/'.$id.'/attachments/'.$fn.'&'.$atchid;
									} else {
										echo 'error';
										reporterror('meefile/editmtsec-attach-uploadhandler.php', 'saving photo attachment taken from upload', 'not able to copy');
									}
									imagedestroy($image_l);
									imagedestroy($image);
					} else {
						echo 'error';
						reporterror('meefile/editmtsec-attach-uploadhandler.php', 'saving  photo attachment taken from upload', 'not correct file type');
					}
