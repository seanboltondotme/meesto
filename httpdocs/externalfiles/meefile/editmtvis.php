...
$mtinfo = mysql_fetch_array (mysql_query ("SELECT u_id, name FROM meefile_tab WHERE mt_id='$mtid' LIMIT 1"), MYSQL_ASSOC);

echo '<div align="left" class="p24" style="width: 460px; border-bottom: 1px solid #C5C5C5;">Edit "'.ucwords($mtinfo['name']).'" Tab Visibility</div>
...
if ($mtinfo['u_id']==$id) { //test for owner

if (isset($_POST['save'])) {
	
	if (isset($_POST['publicvis'])) {
		if (mysql_result (mysql_query("SELECT COUNT(*) FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='pub' AND sub_type IS NOT NULL LIMIT 1"), 0)<1) {
			$addvis = mysql_query("INSERT INTO meefile_tab_vis (mt_id, type, sub_type, time_stamp) VALUES ('$mtid', 'pub', 'y', NOW())");
		}
	} else {
		$delete = mysql_query("DELETE FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='pub'");
	}
	
	if (isset($_POST['streamvis'])) {
		foreach ($_POST['streamvis'] as $streamvis) {
			$streamvis = escape_data($streamvis);
			if (mysql_result (mysql_query("SELECT COUNT(*) FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='strm' AND sub_type='$streamvis' LIMIT 1"), 0)<1) {
				$addvis = mysql_query("INSERT INTO meefile_tab_vis (mt_id, type, sub_type, time_stamp) VALUES ('$mtid', 'strm', '$streamvis', NOW())");
			}
		}
	}
	$strms = mysql_query("SELECT sub_type FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='strm'");
	while ($strm = mysql_fetch_array ($strms, MYSQL_ASSOC)) {
		if (!@in_array($strm['sub_type'], $_POST['streamvis'])) {
			$strmdlt = $strm['sub_type'];
			$delete = mysql_query("DELETE FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='strm' AND sub_type='$strmdlt'");
		}
	}
	...
	if (isset($_POST['chanvis'])) {
		foreach ($_POST['chanvis'] as $chanvis) {
			...
			if (mysql_result (mysql_query("SELECT COUNT(*) FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='chan' AND ref_id='$chanvis' LIMIT 1"), 0)<1) {
				$addvis = mysql_query("INSERT INTO meefile_tab_vis (mt_id, type, ref_id, time_stamp) VALUES ('$mtid', 'chan', '$chanvis', NOW())");
			}
		}
	}
	$chans = mysql_query("SELECT ref_id FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='chan'");
	while ($chan = mysql_fetch_array ($chans, MYSQL_ASSOC)) {
		if (!@in_array($chan['ref_id'], $_POST['chanvis'])) {
			$chandlt = $chan['ref_id'];
			$delete = mysql_query("DELETE FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='chan' AND ref_id='$chandlt'");
		}
	}
	...
	if (isset($_POST['peeplenames'])) {
			foreach ($peeple as $uid) {
				$uid = escape_data($uid);
				if (($uid!=0)&&(mysql_result (mysql_query("SELECT COUNT(*) FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='user' AND ref_id='$uid' LIMIT 1"), 0)<1)) {
				$addvis = mysql_query("INSERT INTO meefile_tab_vis (mt_id, type, ref_id, time_stamp) VALUES ('$mtid', 'user', '$uid', NOW())");
			}
		}
	}
	$prsns = mysql_query("SELECT ref_id FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='user'");
	while ($prsn = mysql_fetch_array ($prsns, MYSQL_ASSOC)) {
		if (!@in_array($prsn['ref_id'], $peeple)) {
			$prsndlt = $prsn['ref_id'];
			$delete = mysql_query("DELETE FROM meefile_tab_vis WHERE mt_id='$mtid' AND type='user' AND ref_id='$prsndlt'");
		}
	}
	
	//apply to children
		$contactinfos = mysql_query("SELECT mts_id FROM meefile_tab_sec WHERE mt_id='$mtid'");
		while ($contactinfo = mysql_fetch_array ($contactinfos, MYSQL_ASSOC)) {
			$mtsid = $contactinfo['mts_id'];
			if (isset($_POST['publicvis'])) {
				if (mysql_result (mysql_query("SELECT COUNT(*) FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='pub' AND sub_type IS NOT NULL LIMIT 1"), 0)<1) {
					$addvis = mysql_query("INSERT INTO meefile_tab_sec_vis (mts_id, type, sub_type, time_stamp) VALUES ('$mtsid', 'pub', 'y', NOW())");
				}
			} else {
				$delete = mysql_query("DELETE FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='pub'");
			}
			
			if (isset($_POST['streamvis'])) {
				foreach ($_POST['streamvis'] as $streamvis) {
					$streamvis = escape_data($streamvis);
					if (mysql_result (mysql_query("SELECT COUNT(*) FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='strm' AND sub_type='$streamvis' LIMIT 1"), 0)<1) {
						$addvis = mysql_query("INSERT INTO meefile_tab_sec_vis (mts_id, type, sub_type, time_stamp) VALUES ('$mtsid', 'strm', '$streamvis', NOW())");
					}
				}
			}
			$strms = mysql_query("SELECT sub_type FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='strm'");
			while ($strm = mysql_fetch_array ($strms, MYSQL_ASSOC)) {
				if (!@in_array($strm['sub_type'], $_POST['streamvis'])) {
					$strmdlt = $strm['sub_type'];
					$delete = mysql_query("DELETE FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='strm' AND sub_type='$strmdlt'");
				}
			}
			...
			if (isset($_POST['chanvis'])) {
				foreach ($_POST['chanvis'] as $chanvis) {
					$chanvis = escape_data($chanvis);
					if (mysql_result (mysql_query("SELECT COUNT(*) FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='chan' AND ref_id='$chanvis' LIMIT 1"), 0)<1) {
						$addvis = mysql_query("INSERT INTO meefile_tab_sec_vis (mts_id, type, ref_id, time_stamp) VALUES ('$mtsid', 'chan', '$chanvis', NOW())");
					}
				}
			}
			$chans = mysql_query("SELECT ref_id FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='chan'");
			while ($chan = mysql_fetch_array ($chans, MYSQL_ASSOC)) {
				if (!@in_array($chan['ref_id'], $_POST['chanvis'])) {
					$chandlt = $chan['ref_id'];
					$delete = mysql_query("DELETE FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='chan' AND ref_id='$chandlt'");
				}
			}
			$peeple = explode(",", $_POST['peeplenames']);
			if (isset($_POST['peeplenames'])) {
			foreach ($peeple as $uid) {
				$uid = escape_data($uid);
				if (($uid!=0)&&(mysql_result (mysql_query("SELECT COUNT(*) FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='user' AND ref_id='$uid' LIMIT 1"), 0)<1)) {
						$addvis = mysql_query("INSERT INTO meefile_tab_sec_vis (mts_id, type, ref_id, time_stamp) VALUES ('$mtsid', 'user', '$uid', NOW())");
					}
				}
			}
			$prsns = mysql_query("SELECT ref_id FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='user'");
			while ($prsn = mysql_fetch_array ($prsns, MYSQL_ASSOC)) {
				if (!@in_array($prsn['ref_id'], $peeple)) {
					$prsndlt = $prsn['ref_id'];
					$delete = mysql_query("DELETE FROM meefile_tab_sec_vis WHERE mts_id='$mtsid' AND type='user' AND ref_id='$prsndlt'");
				}
			}
		}
	...
	if (empty($errors)) {
		...
		reporterror('meefile/editmtvis.php', 'editing '.$mtid.' tab settings', $errors);
		echo '</td></tr></table>
		<script type="text/javascript">
	...
	echo '<form action="'.$baseincpat.'externalfiles/meefile/editmtvis.php?id='.$mtid.'" method="post">
