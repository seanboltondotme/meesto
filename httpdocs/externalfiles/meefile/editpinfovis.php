...
echo '<div align="left" class="p24" style="width: 460px; border-bottom: 1px solid #C5C5C5;">Edit "'.ucwords($mcsecinfo['type']).'" Visibility</div>
...
if (isset($_POST['save'])) {
	...
	if (empty($errors)) {
		...
		reporterror('meefile/editpinfovis.php', 'editing contact infor section '.$mpeid, $errors);
		echo '</td></tr></table>
		<script type="text/javascript">
	...
	if (mysql_result (mysql_query("SELECT COUNT(*) FROM meefile_pers_ext_vis WHERE mpe_id='$mpeid' AND type='pub' LIMIT 1"), 0)>0) {
		$ispub = true;
	} else {
		$ispub = false;
	}
	$plstrm = mysql_query("SELECT sub_type FROM meefile_pers_ext_vis WHERE mpe_id='$mpeid' AND type='strm'");
	$plstrms = array();
	while ($plstrminfo = mysql_fetch_array ($plstrm, MYSQL_ASSOC)) {
		array_push($plstrms, $plstrminfo['sub_type']);
	}
	$plchan = mysql_query("SELECT ref_id FROM meefile_pers_ext_vis WHERE mpe_id='$mpeid' AND type='chan'");
	$plchans = array();
	while ($plchaninfo = mysql_fetch_array ($plchan, MYSQL_ASSOC)) {
		array_push($plchans, $plchaninfo['ref_id']);
	}
	
	echo '<form action="'.$baseincpat.'externalfiles/meefile/editpinfovis.php?ifn='.$ifn.'&id='.$mpeid.'" method="post">
