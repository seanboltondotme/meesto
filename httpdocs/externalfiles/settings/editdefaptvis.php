...
$pdrjs = 'var t4 = new TextboxList(\'form_peeplenames_input\', {unique: true, plugins: {autocomplete: {placeholder: \'start typing the name of one of your peeple to receive suggestions\'}}});';
			...
			$prsns = mysql_query("SELECT ref_id FROM defvis_apt WHERE u_id='$id' AND type='user'");
...
if (isset($_GET['p'])) {
	$p = escape_data($_GET['p']);
} else {
	$p = NULL;
}

echo '<div align="left" class="p24" style="width: 460px; border-bottom: 1px solid #C5C5C5;">Edit Photo Tags Visibility</div>
<div align="left" class="subtext" style="padding-left: 20px; padding-top: 2px; padding-bottom: 22px;">Use this to customize the visibility setting for all of your photo tags.</div>';

if (isset($_POST['save'])) {
	
	if (isset($_POST['publicvis'])) {
		if (mysql_result (mysql_query("SELECT COUNT(*) FROM defvis_apt WHERE u_id='$id' AND type='pub' AND sub_type IS NOT NULL LIMIT 1"), 0)<1) {
			$addvis = mysql_query("INSERT INTO defvis_apt (u_id, type, sub_type, time_stamp) VALUES ('$id', 'pub', 'y', NOW())");
		}
	} else {
		$delete = mysql_query("DELETE FROM defvis_apt WHERE u_id='$id' AND type='pub'");
	}
	
	if (isset($_POST['streamvis'])) {
		foreach ($_POST['streamvis'] as $streamvis) {
			$streamvis = escape_data($streamvis);
			if (mysql_result (mysql_query("SELECT COUNT(*) FROM defvis_apt WHERE u_id='$id' AND type='strm' AND sub_type='$streamvis' LIMIT 1"), 0)<1) {
				$addvis = mysql_query("INSERT INTO defvis_apt (u_id, type, sub_type, time_stamp) VALUES ('$id', 'strm', '$streamvis', NOW())");
			}
		}
	}
	$strms = mysql_query("SELECT sub_type FROM defvis_apt WHERE u_id='$id' AND type='strm'");
	while ($strm = mysql_fetch_array ($strms, MYSQL_ASSOC)) {
		if (!@in_array($strm['sub_type'], $_POST['streamvis'])) {
			$strmdlt = $strm['sub_type'];
			$delete = mysql_query("DELETE FROM defvis_apt WHERE u_id='$id' AND type='strm' AND sub_type='$strmdlt'");
		}
	}
	...
	if (isset($_POST['chanvis'])) {
		foreach ($_POST['chanvis'] as $chanvis) {
			...
			if (mysql_result (mysql_query("SELECT COUNT(*) FROM defvis_apt WHERE u_id='$id' AND type='chan' AND ref_id='$chanvis' LIMIT 1"), 0)<1) {
				$addvis = mysql_query("INSERT INTO defvis_apt (u_id, type, ref_id, time_stamp) VALUES ('$id', 'chan', '$chanvis', NOW())");
			}
		}
	}
	$chans = mysql_query("SELECT ref_id FROM defvis_apt WHERE u_id='$id' AND type='chan'");
	while ($chan = mysql_fetch_array ($chans, MYSQL_ASSOC)) {
		if (!@in_array($chan['ref_id'], $_POST['chanvis'])) {
			$chandlt = $chan['ref_id'];
			$delete = mysql_query("DELETE FROM defvis_apt WHERE u_id='$id' AND type='chan' AND ref_id='$chandlt'");
		}
	}
	...
	if (isset($_POST['peeplenames'])) {
			foreach ($peeple as $uid) {
				$uid = escape_data($uid);
				if (($uid!=0)&&(mysql_result (mysql_query("SELECT COUNT(*) FROM defvis_apt WHERE u_id='$id' AND type='user' AND ref_id='$uid' LIMIT 1"), 0)<1)) {
				$addvis = mysql_query("INSERT INTO defvis_apt (u_id, type, ref_id, time_stamp) VALUES ('$id', 'user', '$uid', NOW())");
			}
		}
	}
	$prsns = mysql_query("SELECT ref_id FROM defvis_apt WHERE u_id='$id' AND type='user'");
	while ($prsn = mysql_fetch_array ($prsns, MYSQL_ASSOC)) {
		if (!@in_array($prsn['ref_id'], $peeple)) {
			$prsndlt = $prsn['ref_id'];
			$delete = mysql_query("DELETE FROM defvis_apt WHERE u_id='$id' AND type='user' AND ref_id='$prsndlt'");
		}
	}
	
	//adjust album vis !important
	
	if (empty($errors)) {
		echo '<table cellpadding="0" cellspacing="0"><tr><td align="left">Your visibility settings have been saved.</td></tr></table>
		<script type="text/javascript">';
			if (isset($p)&&($p=='meefile')) {
				if (mysql_result(mysql_query("SELECT COUNT(*) FROM defvis_apt WHERE u_id='$id' AND type!='user' LIMIT 1"), 0)==0) {
					echo 'window.addEvent(\'load\', function() {
						if (!parent.$(\'taggedpics\').hasClass(\'notvissec\')) {
							var newElem = new Element(\'div\', {\'align\': \'center\', \'class\': \'palert\', \'html\': \'this is not visible to anyone!\'});newElem.inject(parent.$(\'taggedpics\'), \'top\');
						}
						parent.$(\'taggedpics\').addClass(\'notvissec\');
					});';
				} else {
					echo 'window.addEvent(\'load\', function() {
						if (parent.$(\'taggedpics\').hasClass(\'notvissec\')) {
							parent.$(\'taggedpics\').getFirst().destroy();
						}
							parent.$(\'taggedpics\').removeClass(\'notvissec\');
					});';
				}	
			}
		...
		reporterror('settings/editdefaptvis.php', 'setting def vis', $errors);
		echo '</td></tr></table>
		<script type="text/javascript">
	...
	if (mysql_result (mysql_query("SELECT COUNT(*) FROM defvis_apt WHERE u_id='$id' AND type='pub' LIMIT 1"), 0)>0) {
		$ispub = true;
	} else {
		$ispub = false;
	}
	$plstrm = mysql_query("SELECT sub_type FROM defvis_apt WHERE u_id='$id' AND type='strm'");
	$plstrms = array();
	while ($plstrminfo = mysql_fetch_array ($plstrm, MYSQL_ASSOC)) {
		array_push($plstrms, $plstrminfo['sub_type']);
	}
	$plchan = mysql_query("SELECT ref_id FROM defvis_apt WHERE u_id='$id' AND type='chan'");
	$plchans = array();
	while ($plchaninfo = mysql_fetch_array ($plchan, MYSQL_ASSOC)) {
		array_push($plchans, $plchaninfo['ref_id']);
	}
	
	echo '<form action="'.$baseincpat.'externalfiles/settings/editdefaptvis.php'; if (isset($p)){echo'?p='.$p;} echo'" method="post">
