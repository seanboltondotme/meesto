				...
				if (in_array($mimetest['mime'], $allowedmime)) {
						...
						$pfn = strtolower(substr($tmpfn, 0, strrpos($tmpfn, '.')));
									...
									if (move_uploaded_file($filename, "../../users/$id/attachments/$fn")) {
										...
										echo 'postfeed'.$id.'&users/'.$id.'/attachments/'.$fn.'&'.$atchid;
									} else {
										echo 'error';
										reporterror('home/postfeed-attach-uploadhandler.php', 'saving  photo attachment taken from upload', 'not able to copy');
									}
									imagedestroy($image_l);
									imagedestroy($image);
					} else {
						echo 'error';
						reporterror('home/postfeed-attach-uploadhandler.php', 'saving  photo attachment taken from upload', 'not correct file type');
					}
