...
if (isset($_POST['save'])) {
		...
		$pdrjs = 'parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'vis[all]\').set(\'checked\', false);
		parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'vis[cust]\').set(\'checked\', true);';
		
		if (isset($_POST['publicvis'])) {
			$pdrjs .= 'parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'publicvis\').set(\'checked\', true);';
		} else {
			$pdrjs .= 'parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'publicvis\').set(\'checked\', false);';
		}
		...
		foreach ($strmlst as $strmname) {
			if (in_array($strmname, $_POST['streamvis'])) {
				$pdrjs .= 'parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'streamvis['.$strmname.']\').set(\'checked\', true);';
			} else {
				$pdrjs .= 'parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'streamvis['.$strmname.']\').set(\'checked\', false);';	
			}
		}
		...
		while ($chaninfo = mysql_fetch_array ($chanlst, MYSQL_ASSOC)) {
			if (in_array($chaninfo['mpc_id'], $_POST['chanvis'])) {
				$pdrjs .= 'parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'chanvis['.$chaninfo['mpc_id'].']\').set(\'checked\', true);';
			} else {
				$pdrjs .= 'parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'chanvis['.$chaninfo['mpc_id'].']\').set(\'checked\', false);';	
			}
		}
		if (isset($_POST['peeplenames'])) {
			$pdrjs .= 'parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'form_peeplenames_input\').set(\'value\', \''.$_POST['peeplenames'].'\');';
		} else {
			$pdrjs .= 'parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'form_peeplenames_input\').set(\'value\', \'\');';
		}
	
} else {
	$pdrjs = 'if (parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'publicvis\').get(\'checked\') == true){
				...
				$(\'streamvis[mb]\').set(\'checked\', parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'streamvis[mb]\').get(\'checked\') );
				$(\'streamvis[frnd]\').set(\'checked\', parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'streamvis[frnd]\').get(\'checked\') );
				$(\'streamvis[fam]\').set(\'checked\', parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'streamvis[fam]\').get(\'checked\') );
				$(\'streamvis[prof]\').set(\'checked\', parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'streamvis[prof]\').get(\'checked\') );
				$(\'streamvis[edu]\').set(\'checked\', parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'streamvis[edu]\').get(\'checked\') );
				$(\'streamvis[aqu]\').set(\'checked\', parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'streamvis[aqu]\').get(\'checked\') );';
	...
	while ($plchan = @mysql_fetch_array ($plchans, MYSQL_ASSOC)) {
		$pdrjs .= '$(\'chanvis['.$plchan['mpc_id'].']\').set(\'checked\', parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'chanvis['.$plchan['mpc_id'].']\').get(\'checked\') );';
	}
	$pdrjs .= 'var t4 = new TextboxList(\'form_peeplenames_input\', {unique: true, plugins: {autocomplete: {placeholder: \'start typing the name of one of your peeple to receive suggestions\'}}});
			...
			new Request.JSON({url: \''.$baseincpat.'externalfiles/autocompleter/grabmypeeple.php\', onSuccess: function(r){
				t4.plugins[\'autocomplete\'].setValues(r);
					var blkpeeps = parent.$(\'feedwritecmt'.$fid.'\').contentWindow.$(\'form_peeplenames_input\').get(\'value\').split(\',\');
...
echo '<div align="left" class="p24" style="width: 460px; border-bottom: 1px solid #C5C5C5;">Edit Feed Comment Visibility</div>
...
if (isset($_POST['save'])) {
	...
	if (empty($errors)) {
		...
		reporterror('home/editpostcmtvis.php', 'default vis settings', $errors);
		echo '</td></tr></table>
		<script type="text/javascript">
	...
	echo '<form action="'.$baseincpat.'externalfiles/home/editpostcmtvis.php?id='.$fid.'" method="post">
		...
		<div align="center" id="sbmtbtns">
			<div align="center" style="padding-top: 16px;">
				...
				note: your comment will not be more visible than the main post
			</div>
		</div>
