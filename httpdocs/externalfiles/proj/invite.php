...
<div align="left" class="subtext" style="padding-left: 20px; padding-top: 2px; padding-bottom: 18px;">Use this to invite peeple to "'.$cpinfo['name'].'"</div>';

if (mysql_result(mysql_query("SELECT COUNT(*) FROM commproj_mem WHERE cp_id='$cpid' AND u_id='$id' LIMIT 1"), 0)>0) {//test for mem
...
if (isset($_POST['invite'])) {
	...
	if (empty($errors)) {
		foreach ($_POST['peeple'] as $peepid) {
			...
			if ((mysql_result(mysql_query("SELECT COUNT(*) FROM commproj_mem WHERE cp_id='$cpid' AND u_id='$peepid'"), 0)==0)|| (mysql_result(mysql_query("SELECT COUNT(*) FROM requests WHERE u_id='$peepid' AND type='invtcproj' AND ref_id='$cpid'"), 0)==0)) {
				$insert = mysql_query("INSERT INTO requests (u_id, type, s_id, ref_id, time_stamp) VALUES ('$peepid', 'invtcproj', '$id', '$cpid', NOW())");
			}
		}
	...
	echo '<form action="'.$baseincpat.'externalfiles/proj/invite.php?id='.$cpid.'" method="post">
		...
		<div align="left" id="peeparea" style="padding-left: 16px; padding-bottom: 12px; height: 130px; width: 644px; overflow-x: none; overflow-y: scroll;"">';
			...
			while ($person = mysql_fetch_array ($peeple, MYSQL_ASSOC)) {
				$pid = $person['p_id'];
				if ((mysql_result(mysql_query("SELECT COUNT(*) FROM commproj_mem WHERE cp_id='$cpid' AND u_id='$pid'"), 0)>0)|| (mysql_result(mysql_query("SELECT COUNT(*) FROM requests WHERE u_id='$pid' AND type='invtcproj' AND ref_id='$cpid'"), 0)>0)) {
	...
	echo '<div class="container" align="left" valign="top" style="padding: 24px;">
		You must be a supporter of a project to be able to invite peeple.
	</div>';
}
