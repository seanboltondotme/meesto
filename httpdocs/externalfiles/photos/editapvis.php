...
echo '<div align="left" class="p24" style="width: 460px; border-bottom: 1px solid #C5C5C5;">Edit Album Photo Visibility</div>
...
if (mysql_result(mysql_query("SELECT COUNT(*) FROM photo_albums pa INNER JOIN album_photos ap ON ap.pa_id=pa.pa_id AND ap.ap_id='$apid' WHERE pa.u_id='$id' LIMIT 1"), 0)>0) { //test for owner
...
if (isset($_POST['save'])) {
	...
	//update parent
	$paid = mysql_result(mysql_query("SELECT pa_id FROM album_photos WHERE ap_id='$apid' LIMIT 1"), 0);
		if (isset($_POST['publicvis'])) {
	...
	if (empty($errors)) {
		...
		reporterror('photos/editapvis.php', 'editing apid='.$apid.' vis', $errors);
		echo '</td></tr></table>
		<script type="text/javascript">
	...
	if (mysql_result (mysql_query("SELECT COUNT(*) FROM album_photos_vis WHERE ap_id='$apid' AND type='pub' LIMIT 1"), 0)>0) {
		$ispub = true;
	} else {
		$ispub = false;
	}
	$plstrm = mysql_query("SELECT sub_type FROM album_photos_vis WHERE ap_id='$apid' AND type='strm'");
	$plstrms = array();
	while ($plstrminfo = mysql_fetch_array ($plstrm, MYSQL_ASSOC)) {
		array_push($plstrms, $plstrminfo['sub_type']);
	}
	$plchan = mysql_query("SELECT ref_id FROM album_photos_vis WHERE ap_id='$apid' AND type='chan'");
	$plchans = array();
	while ($plchaninfo = mysql_fetch_array ($plchan, MYSQL_ASSOC)) {
		array_push($plchans, $plchaninfo['ref_id']);
	}
	
	echo '<form action="'.$baseincpat.'externalfiles/photos/editapvis.php?id='.$apid.'" method="post">
