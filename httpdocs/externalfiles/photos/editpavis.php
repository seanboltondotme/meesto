...
$pdrjs = 'var t4 = new TextboxList(\'form_peeplenames_input\', {unique: true, plugins: {autocomplete: {placeholder: \'start typing the name of one of your peeple to receive suggestions\'}}});';
			...
			$prsns = mysql_query("SELECT ref_id FROM photo_album_vis WHERE pa_id='$paid' AND type='user'");
...
$ainfo = mysql_fetch_array (mysql_query ("SELECT u_id, name FROM photo_albums WHERE pa_id='$paid' LIMIT 1"), MYSQL_ASSOC);

echo '<div align="left" class="p24" style="border-bottom: 1px solid #C5C5C5;">Edit "'.ucwords($ainfo['name']).'" Album Visibility</div>
...
if ($ainfo['u_id']==$id) { //test for owner
...
if (isset($_POST['save'])) {
	...
	if (isset($_POST['publicvis'])) {
		...
		$delete = mysql_query("DELETE FROM photo_album_vis WHERE pa_id='$paid' AND type='pub'");
	}
	...
	$strms = mysql_query("SELECT sub_type FROM photo_album_vis WHERE pa_id='$paid' AND type='strm'");
	while ($strm = mysql_fetch_array ($strms, MYSQL_ASSOC)) {
		if (!@in_array($strm['sub_type'], $_POST['streamvis'])) {
			$strmdlt = $strm['sub_type'];
			$delete = mysql_query("DELETE FROM photo_album_vis WHERE pa_id='$paid' AND type='strm' AND sub_type='$strmdlt'");
		}
	}
	...
	$chans = mysql_query("SELECT ref_id FROM photo_album_vis WHERE pa_id='$paid' AND type='chan'");
	while ($chan = mysql_fetch_array ($chans, MYSQL_ASSOC)) {
		if (!@in_array($chan['ref_id'], $_POST['chanvis'])) {
			$chandlt = $chan['ref_id'];
			$delete = mysql_query("DELETE FROM photo_album_vis WHERE pa_id='$paid' AND type='chan' AND ref_id='$chandlt'");
		}
	}
	...
	while ($prsn = mysql_fetch_array ($prsns, MYSQL_ASSOC)) {
		if (!@in_array($prsn['ref_id'], $peeple)) {
			$prsndlt = $prsn['ref_id'];
			$delete = mysql_query("DELETE FROM photo_album_vis WHERE pa_id='$paid' AND type='user' AND ref_id='$prsndlt'");
		}
	}
	
	//apply to children
		$contactinfos = mysql_query("SELECT ap_id FROM album_photos WHERE pa_id='$paid'");
		while ($contactinfo = mysql_fetch_array ($contactinfos, MYSQL_ASSOC)) {
			$apid = $contactinfo['ap_id'];
			if (isset($_POST['publicvis'])) {
				if (mysql_result (mysql_query("SELECT COUNT(*) FROM album_photos_vis WHERE ap_id='$apid' AND type='pub' AND sub_type IS NOT NULL LIMIT 1"), 0)<1) {
					$addvis = mysql_query("INSERT INTO album_photos_vis (ap_id, type, sub_type, time_stamp) VALUES ('$apid', 'pub', 'y', NOW())");
				}
			} else {
				$delete = mysql_query("DELETE FROM album_photos_vis WHERE ap_id='$apid' AND type='pub'");
			}
			
			if (isset($_POST['streamvis'])) {
				foreach ($_POST['streamvis'] as $streamvis) {
					$streamvis = escape_data($streamvis);
					if (mysql_result (mysql_query("SELECT COUNT(*) FROM album_photos_vis WHERE ap_id='$apid' AND type='strm' AND sub_type='$streamvis' LIMIT 1"), 0)<1) {
						$addvis = mysql_query("INSERT INTO album_photos_vis (ap_id, type, sub_type, time_stamp) VALUES ('$apid', 'strm', '$streamvis', NOW())");
					}
				}
			}
			$strms = mysql_query("SELECT sub_type FROM album_photos_vis WHERE ap_id='$apid' AND type='strm'");
			while ($strm = mysql_fetch_array ($strms, MYSQL_ASSOC)) {
				if (!@in_array($strm['sub_type'], $_POST['streamvis'])) {
					$strmdlt = $strm['sub_type'];
					$delete = mysql_query("DELETE FROM album_photos_vis WHERE ap_id='$apid' AND type='strm' AND sub_type='$strmdlt'");
				}
			}
			...
			if (isset($_POST['chanvis'])) {
				foreach ($_POST['chanvis'] as $chanvis) {
					$chanvis = escape_data($chanvis);
					if (mysql_result (mysql_query("SELECT COUNT(*) FROM album_photos_vis WHERE ap_id='$apid' AND type='chan' AND ref_id='$chanvis' LIMIT 1"), 0)<1) {
						$addvis = mysql_query("INSERT INTO album_photos_vis (ap_id, type, ref_id, time_stamp) VALUES ('$apid', 'chan', '$chanvis', NOW())");
					}
				}
			}
			$chans = mysql_query("SELECT ref_id FROM album_photos_vis WHERE ap_id='$apid' AND type='chan'");
			while ($chan = mysql_fetch_array ($chans, MYSQL_ASSOC)) {
				if (!@in_array($chan['ref_id'], $_POST['chanvis'])) {
					$chandlt = $chan['ref_id'];
					$delete = mysql_query("DELETE FROM album_photos_vis WHERE ap_id='$apid' AND type='chan' AND ref_id='$chandlt'");
				}
			}
			$peeple = explode(",", $_POST['peeplenames']);
			if (isset($_POST['peeplenames'])) {
			foreach ($peeple as $uid) {
				$uid = escape_data($uid);
				if (($uid!=0)&&(mysql_result (mysql_query("SELECT COUNT(*) FROM album_photos_vis WHERE ap_id='$apid' AND type='user' AND ref_id='$uid' LIMIT 1"), 0)<1)) {
						$addvis = mysql_query("INSERT INTO album_photos_vis (ap_id, type, ref_id, time_stamp) VALUES ('$apid', 'user', '$uid', NOW())");
					}
				}
			}
			$prsns = mysql_query("SELECT ref_id FROM album_photos_vis WHERE ap_id='$apid' AND type='user'");
			while ($prsn = mysql_fetch_array ($prsns, MYSQL_ASSOC)) {
				if (!@in_array($prsn['ref_id'], $peeple)) {
					$prsndlt = $prsn['ref_id'];
					$delete = mysql_query("DELETE FROM album_photos_vis WHERE ap_id='$apid' AND type='user' AND ref_id='$prsndlt'");
				}
			}
		}
	...
	if (empty($errors)) {
		...
		reporterror('photos/editpavis.php', 'editing apid='.$paid.' vis', $errors);
		echo '</td></tr></table>
		<script type="text/javascript">
	...
	if (mysql_result (mysql_query("SELECT COUNT(*) FROM photo_album_vis WHERE pa_id='$paid' AND type='pub' LIMIT 1"), 0)>0) {
		$ispub = true;
	} else {
		$ispub = false;
	}
	$plstrm = mysql_query("SELECT sub_type FROM photo_album_vis WHERE pa_id='$paid' AND type='strm'");
	$plstrms = array();
	while ($plstrminfo = mysql_fetch_array ($plstrm, MYSQL_ASSOC)) {
		array_push($plstrms, $plstrminfo['sub_type']);
	}
	$plchan = mysql_query("SELECT ref_id FROM photo_album_vis WHERE pa_id='$paid' AND type='chan'");
	$plchans = array();
	while ($plchaninfo = mysql_fetch_array ($plchan, MYSQL_ASSOC)) {
		array_push($plchans, $plchaninfo['ref_id']);
	}
	
	echo '<form action="'.$baseincpat.'externalfiles/photos/editpavis.php?id='.$paid.'" method="post">
		...
		<div align="center" id="sbmtbtns">
			<div align="center" style="padding-top: 16px;">
				...
				note: these settings will be applied to all photos in this album
			</div>
		</div>
