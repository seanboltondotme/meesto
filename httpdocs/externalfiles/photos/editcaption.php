...
$apinfo = mysql_fetch_array(mysql_query ("SELECT pa.u_id, ap.caption FROM photo_albums pa INNER JOIN album_photos ap ON ap.pa_id=pa.pa_id AND ap.ap_id='$apid' LIMIT 1"), MYSQL_ASSOC);
...
if (isset($_POST['save'])) {
	...
	if (empty($errors)) {
		$update = mysql_query("UPDATE album_photos SET caption='$description' WHERE ap_id='$apid'");
		
		echo '<div align="center" class="p18">Your caption has been saved.</div>
		<script type="text/javascript">
			setTimeout("parent.gotopage(\'caption_cont\', \''.$baseincpat.'externalfiles/photos/grabcaption.php?id='.$apid.'\');", 0);
	...
	echo '<form action="'.$baseincpat.'externalfiles/photos/editcaption.php?id='.$apid.'" method="post">
